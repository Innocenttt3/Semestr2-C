...
void* printPart2(void *arg) {
    for (int y = sizeOfArray/2 + 1; y < sizeOfArray; y++ ) {
        arr[y] = b;
    }
    return NULL;
}
...
int main() {
    ...
    scanf("podaj wielkosc tablicy: %d \n", &sizeOfArray);
    arr = (int*)malloc(sizeOfArray * sizeof(int));

    scanf("podaj liczbe do pierwszej czesci: %d \n", &a);
    scanf("podaj liczbe do drugiej czesci: %d \n", &b);
    
    if (pthread_create(&writerPart1, NULL, multi, NULL)) {
        printf("błąd tworzenia wątku.");
        abort();
    }
    if (pthread_create(&writerPart2, NULL, multi, NULL)) {
        printf("błąd tworzenia wątku.");
        abort();
    }
